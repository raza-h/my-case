...
namespace AbsolCase.Areas.Staff.Controllers
{
    ...
    public class StaffEventController : Controller
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        public StaffEventController(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public IActionResult AddUpdateEvents(Events model)
        {
            try
            {
                ...
                SResponse resp = AbsolCase.Configuration.RequestSender.Fetch.GotoService("api", "Events/AddUpdateEvents", "POST", body);
                if (resp.Status && (resp.Resp != null) && (resp.Resp != ""))
                {
                    return Json("Added");
                }
        ...
        public IActionResult DeleteEvents(int Id)
        {
            try
            {
                SResponse resp = AbsolCase.Configuration.RequestSender.Fetch.GotoService("api", $"Events/DeleteEvents?Id={Id}", "DELETE");
                return Json("Ok");
            }
            catch (Exception ex)
            {
